...
class UnixFileSystem extends FileSystem {
    ...
    public int prefixLength(String pathname) {
        ...
        /* AmigaOS4: "VOLUME:path" (volume or assign name followed by ':') is
           absolute -- the prefix is everything up to and including ':'.
           Without this, File.isAbsolute("JAVA:...") is false and paths get
           wrongly resolved against user.dir (e.g. "Work:Java/JAVA:libzip.so"). */
        int colon = pathname.indexOf(':');
        int slash = pathname.indexOf('/');
        if (colon > 0 && (slash < 0 || colon < slash)) return colon + 1;
        return 0;
    }
    ...
    public String resolve(String parent, String child) {
        ...
        /* AmigaOS4: a volume/assign name ends in ':' and IS the directory --
           joining with '/' would reference the parent of the volume root
           (e.g. "JAVA:/libzip.so" is invalid; "JAVA:libzip.so" is right). */
        if (parent.charAt(parent.length() - 1) == ':') return parent + child;
        return parent + '/' + child;
    }
    ...
    /* Required by newer OpenJDK 8 updates (Temurin): FileSystem declares this
       abstract.  Same implementation as upstream JDK 8u. */
