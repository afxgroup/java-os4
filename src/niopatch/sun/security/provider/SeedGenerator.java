...
abstract class SeedGenerator {
    ...
    static {
         ...
         * missing the old behaviour is still there, slow but working.
         */
        if (instance == null) {
            try {
                instance = new AmigaSeedGenerator();
                if (debug != null) {
                    debug.println("Using AmigaOS getentropy seed generator");
                }
            } catch (Throwable t) {
                if (debug != null) {
                    debug.println("getentropy unavailable: " + t);
                }
            }
        }
    ...
    /**
     * Seed bytes from clib4's getentropy(), in src/openjdk/amiga_crypto.c.
     *
     * The constructor asks for one byte so that a runtime without the native
     * fails here, at selection time, rather than at the first seed request --
     * by which point the caller is already inside a TLS handshake.
     */
    private static final class AmigaSeedGenerator extends SeedGenerator {

        AmigaSeedGenerator() {
            byte[] probe = new byte[1];

            if (!nativeSeed(probe, probe.length)) {
                throw new UnsupportedOperationException("getentropy unavailable");
            }
        }

        @Override
        void getSeedBytes(byte[] result) {
            if (!nativeSeed(result, result.length)) {
                /* Never silently return an unfilled buffer: a caller reading
                   zeros as entropy is worse than a slow one. */
                throw new InternalError("getentropy failed after succeeding once");
            }
        }
    }

    private static native boolean nativeSeed(byte[] result, int len);
