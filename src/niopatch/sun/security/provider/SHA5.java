...
abstract class SHA5 extends DigestBase {
    ...
    final void implCompress(byte[] buf, int ofs) {
        ...
        implCompressNative(buf, ofs, state);
    }

    /*
     * AmigaOS: the compression function is native, in src/openjdk/amiga_crypto.c.
     *
     * implCompressCheck still runs first, so the bounds work and the big-endian
     * expansion into W are unchanged -- the native re-reads the block from buf
     * rather than trusting W, which costs 16 loads against 80 rounds and keeps
     * the two halves independent.
     *
     * No visibility trouble here, unlike GHASH: sun.security.provider is in
     * rt.jar, boot-loaded, the same loader niopatch.zip is on, so this class and
     * its DigestBase superclass stay in one runtime package.
     */
    private static native void implCompressNative(byte[] buf, int ofs, long[] state);
