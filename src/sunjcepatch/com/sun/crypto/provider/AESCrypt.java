...
final class AESCrypt extends SymmetricCipher implements AESConstants
{
    ...
    /**
     * AmigaOS: the key GCTR hands to its native counter-mode loop.
     *
     * Returned rather than the expanded sessionK because the native does its
     * own expansion -- once per TLS record, against a thousand blocks, which
     * costs nothing and keeps the two schedules from having to agree on a
     * layout.
     */
    byte[] amigaRawKey() {
        return lastKey;
