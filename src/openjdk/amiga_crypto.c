...
Java_java_lang_AmigaDiag_cryptoStats(JNIEnv *env, jclass clazz) {
    ...
    (void)clazz;
    /* GHASH only.  GCTR's counters cannot live here: its class comes from the
       extension loader, so its native must be in a library that loader owns --
       libamigacrypto.so -- and a separate .so has separate statics.  See
       GCTR.nativeStats(). */
    v[0] = (jlong)ghash_calls;
    v[1] = (jlong)ghash_blocks;

    if ((out = (*env)->NewLongArray(env, 2)) != NULL) {
